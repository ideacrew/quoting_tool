...
import {
  TieredContributionModel,
  RelationshipContributionModel,
} from '../data/contribution_models'
import {
  ClientPreferences,
  CLIENT_PREFERENCES,
  PackageTypes,
...
export class PlanFilterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.employerDetails) {
      ...
      this.tieredCalculator = this.calculator(
        startDate,
        this.tieredContributionModel,
        true
      )
  ...
  changePackageFilter(evt) {
    ...
    this.hasRelationshipCompatibleType = false
    if (newVal != null) {
      this.hasRelationshipCompatibleType = this.isRelationshipPackageType(
        this.planFilter
      )
  ...
  recalculate() {
    const calculator = this.hasRelationshipCompatibleType
      ? this.relationshipCalculator
      : this.tieredCalculator
    const newQuotes = calculator.quoteProducts(
    ...
    const filteredQuotes = newQuotes.filter(function(nq) {
      return fProductsForCompare.includes(
        nq.product_information.name + nq.product_information.provider_name
      )
    })
        ...
        this.selectedInsuranceCompanies
          ...
          .map(() => {
            ;[...this.selectedMetalLevels].filter(ml => {
              multiArray.filter(
                array => array['product_information']['metal_level'] === ml
              )
            })
          })
      }
        ...
        this.selectedInsuranceCompanies
          ...
          .map(() => {
            ;[...this.selectedProductTypes].filter(product => {
              multiArray.filter(
                array =>
                  array['product_information']['product_type'] === product
              )
            })
          })
      }
  ...
  metalLevelCount(metalLevel, planType) {
    if (planType === 'health') {
      const count = this.filteredCarriers.filter(
        plan => plan['product_information']['metal_level'] === metalLevel
      )
  ...
  productTypeCounts(product) {
    const count = this.filteredCarriers.filter(
      plan => plan['product_information']['product_type'] === product
    )
    return `(${count.length} Plans)`
  }
  ...
  hsaCounts(hsa) {
    const count = this.filteredCarriers.filter(
      plan => plan['product_information']['hsa_eligible'] === hsa
    )
    return `(${count.length} Plans)`
  }
