...
export class IssuerBucket {
  private expensiveFamilies: Map<String, CostResult> = new Map<
    String,
    CostResult
  >()
  ...
  add(product: Product, re: RosterEntry, total: number): void {
    if (this.expensiveFamilies.has(product.provider_name)) {
      const currentCostResult = this.expensiveFamilies.get(
        product.provider_name
      )
      if (currentCostResult.total < total) {
        this.expensiveFamilies.set(
          product.provider_name,
          new CostResult(re, total)
        )
      }
    } else {
      this.expensiveFamilies.set(
