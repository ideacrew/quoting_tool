...
module Api
  module V1
    # which handles products related actions
    class ProductsController < ApplicationController
      ...
      def plans
        ...
        data = Rails.cache.fetch("data_#{kind}_#{county}_#{zip}_#{sic}_#{year}_#{month}", expires_in: 45.minutes) do
          service_area_ids = service_area_ids(county, zip, year)
          period = Date.new(year, 1, 1).end_of_year
          products = Products::Product.where(:kind => kind, :service_area_id.in => service_area_ids, :"application_period.min".lte => effective_date, :"application_period.max".gte => period)
