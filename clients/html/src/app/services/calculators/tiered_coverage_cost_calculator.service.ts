...
class FilteredRelationshipRosterEntry {
  ...
  private setBucketAndDeps(start_date, deps, rel_map, allowed_buckets) {
    ...
    this.roster_dependents = this.filterDependents(
      start_date,
      clean_deps,
      rel_map
    )
  ...
  private selectBucket(
    ...
    if (
      remain_to_pick.indexOf(ContributionTierName.EMPLOYEE_AND_DEPENDENTS) >
        -1 &&
      (rels.indexOf(ContributionRelationship.SPOUSE) > -1 ||
        rels.indexOf(ContributionRelationship.DOMESTIC_PARTNER) > -1)
    ) {
...
class BucketCount {
  ...
  public toLevels(product) {
    ...
    bucket_map.set(
      ...
      reduced_value *
        product.group_tier_factor(ContributionTierName.EMPLOYEE_ONLY)
    )
    bucket_map.set(
      ContributionTierName.EMPLOYEE_AND_SPOUSE,
      reduced_value *
        product.group_tier_factor(ContributionTierName.EMPLOYEE_AND_SPOUSE)
    )
    bucket_map.set(
      ContributionTierName.EMPLOYEE_AND_DEPENDENTS,
      reduced_value *
        product.group_tier_factor(ContributionTierName.EMPLOYEE_AND_DEPENDENTS)
    )
    bucket_map.set(
