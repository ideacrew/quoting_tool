...
export class EmployerDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    this.selectedSicService.currentMessage.subscribe(message =>
      this.setSicFromTree(message)
    )
    ...
    if (this.employeeRoster) {
      ...
      this.quoteForm
        .get('effectiveDate')
        .setValue(new Date(Date.parse(this.employerDetails.effectiveDate)))
      this.quoteForm.get('zip').setValue(this.employerDetails.zip.zipCode)
      this.quoteForm
        .get('sic')
        .setValue(this.employerDetails.sic.standardIndustryCodeCode)
    ...
    if (this.todaysDate.getMonth() + 1 > 11) {
      ...
      this.effectiveDateOptions = [
        { month: -1, value: 'SELECT DATE', disabled: true },
        {
          ...
          value: `${
            this.months[this.todaysDate.getMonth()]
      ...
      this.effectiveDateOptions = [
        ...
        },
        {
          ...
          value: `${
            this.months[this.todaysDate.getMonth() + 1]
          } ${this.todaysDate.getFullYear()}`,
        },
      ]
    }
  ...
  setSicFromTree(item) {
    if (item !== 'default item') {
      const sicValue = this.sics.filter(
        sic => sic['standardIndustryCodeFull'] === item.text
      )[0]['standardIndustryCodeCode']
  ...
  isSelected(date) {
    if (
      ...
      date.value === this.employerDetails.effectiveDate
    ) {
      return true
    } else {
      return false
  ...
  createRoster() {
    ...
    localStorage.setItem(
      'employerDetails',
      JSON.stringify(this.quoteForm.value)
    )
  ...
  removeEmployeeFromRoster(rowIndex) {
    ...
    localStorage.setItem(
      'employerDetails',
      JSON.stringify(this.employerDetails)
    )
  }
